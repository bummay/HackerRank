...
def splitCoins(val, coins, coinList, ret, dict):
    ...
    for coin in splitted:
        if coin == min(coins) and val % coin == 0:
        ...
        isSet = False
        change = val - coin
        dictChange = dict[change]
        dictCoinChange = dict[(coin,change)]

        if len([i for i in coins if i < change]) > 0:
            maxCoin = max([i for i in coins if i <= change])
        else:
            maxCoin = 0
        isGreqThanMaxCoin = (coin >= maxCoin and maxCoin != min(coins))

        if isGreqThanMaxCoin and dictChange > 0:
            ret += dictChange

        elif isGreqThanMaxCoin != True and dictCoinChange > 0:
            ret += dict[(coin, change)]

        elif dictChange >= 0 and dict[(coin, change)] >= 0:
            result = 0
            coinList += [coin]
            if dictChange == 0:
                if isGreqThanMaxCoin:
                    isSet = True
                if isSet and change in coins:
                    result += 1
            result += splitCoins(change, coins, coinList, 0, dict)
...
if __name__ == '__main__':
    ...
    for i in range(len(inputs)):
        ...
        print(str(i) + ":" + str(inputs[i][0]) + ":" + str(result) + ":" + str(isTrue))
